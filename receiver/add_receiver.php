...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $stmt = mysqli_prepare($conn, 'SELECT id FROM receivers WHERE contact=? AND blood_group=? AND receive_date=? LIMIT 1');
        mysqli_stmt_bind_param($stmt, 'sss', $contact, $blood_group, $date);
        mysqli_stmt_execute($stmt);
        if (mysqli_fetch_assoc(mysqli_stmt_get_result($stmt))) {
            $errors[] = 'Duplicate receiver record already exists.';
        } else {
            $stmt = mysqli_prepare($conn, 'INSERT INTO receivers (name, contact, receive_date, blood_group) VALUES (?, ?, ?, ?)');
            mysqli_stmt_bind_param($stmt, 'ssss', $name, $contact, $date, $blood_group);
            if (mysqli_stmt_execute($stmt)) {
                flash_set('success', 'Receiver added successfully.');
...
?>
<h2 class="page-title">Add Receiver</h2>
...
<form method="POST" class="form-card">
    ...
    <a class="btn" href="<?php echo BASE_URL; ?>receiver/receiver.php">Back</a>
</form>
